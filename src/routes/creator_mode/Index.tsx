...
export default function Index() {
    ...
    return (
        <>
            <div className={`${menuOn ? 'opacity-100 delay-200' : 'opacity-0'} font container absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center min-w-[78vw] max-w-fit`}>
                
                <div className="space-y-2 w-[21vh] mr-28">
                        <button onClick={() => openCreatorMode('races')} className="bg-bg-1/50 hover:bg-bg-1/65 transition-colors p-2 w-full rounded-md text-sm block">
                            Races
                        </button>
                        <button onClick={() => openCreatorMode('hosts')} className="bg-bg-1/50 hover:bg-bg-1/65 transition-colors p-2 w-full rounded-md text-sm block">
                            Hosts
                        </button>
                        <button onClick={() => openCreatorMode('creator_mode')} className="bg-bg-1/80 hover:bg-bg-1/65 transition-colors p-2 w-full rounded-md text-sm block">
                            Creator mode
                        </button>
                        <button onClick={() => openCreatorMode('credits')} className="bg-bg-1/50 hover:bg-bg-1/65 transition-colors p-2 w-full rounded-md text-sm block">
                            Credits
                        </button>
                    </div>   
                        ...
                        <div className="">
                            ...
                            <div className="text-sm">
                                ...
                                <div className="flex mb-8 bg-bg-1/80 w-fit pl-2 rounded-sm items-center  text-fg-1/50 mt-2 ml-auto mr-auto">
                                    <FaSearch />
                                    <input type="text" className="ml-2 text-sm bg-[rgba(0,0,0,0)] pr-3 outline-none p-1" placeholder="Search map" onChange={handleChange} value={searchBar} />
                                </div>
                                    ...
                                    <div className="rounded-sm overflow-hidden max-h-[30vh] overflow-y-auto pr-2 w-[34vh]">
                                        {filteredMaps.map((map, index) => (
                                            <div key={index} className="flex bg-bg-1/60 p-2 items-center">
                                                ...
                                                <button onClick={() => editMap(map.id)} className="ml-auto text-xs bg-bg-1/50 p-1 h-fit rounded-sm hover:bg-bg-1/70 transition-colors"><FaEdit /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="">
                                ...
                                <div className="mt-2">
                                    <div className="flex mb-2 bg-bg-1/80 w-fit pl-2 rounded-sm items-center text-fg-1/50 ml-auto mr-auto">
                                        <BiRename />
                                        <input type="text" className="ml-2 text-sm bg-[rgba(0,0,0,0)] pr-3 outline-none p-1" value={nameMap} onChange={handleChangeMap} placeholder="Map name" />
                                    </div>
                                    ...
                                    <div className="mt-8">
                                        ...
                                        <div className="mt-8 text-sm flex">
                                            <div className="space-x-8 ml-auto mr-auto">
                                                <button onClick={() => cancelMap()} className=" p-1 pl-4 pr-4 rounded-sm bg-bg-1/60 hover:bg-bg-1/70 transition-colors active:bg-bg-1/90">Cancel</button>
                                                <button onClick={() => createMap()} className="bg-accent-1/60 p-1 pl-4 pr-4 rounded-sm hover:bg-accent-1/75 transition-colors active:bg-accent-1">Create</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div className={`${menuOn ? 'opacity-100 delay-200' : 'opacity-0'} font z-10 absolute top-[10vh] right-[6vh] -translate-x-1/2 -translate-y-1/2 font text-[1.6vh] text-bg-1 uppercase bg-fg-1 text-fix rounded-md border-[0.5vh] border-bg-3/50`}>
                <p>esc</p>
            </div>
